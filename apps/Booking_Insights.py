...
#     app,
#     {'simplyOR': 'simplyOR@2023'}
# )

# app.title = "Booking Insights Dashboard"

columns = ['Route', 'Flight No.', 'Departure Time', 'Days-to-Departure', 'Forecast', 'Current Booked',
           'Last 3 day Bookings']

df = pd.read_csv(DATA_PATH.joinpath("forward_load_analytics.csv"))
df_product = pd.read_csv(DATA_PATH.joinpath("Product.csv"))
df_journey = pd.read_csv(DATA_PATH.joinpath("Journey.csv"))
df_channel = pd.read_csv(DATA_PATH.joinpath("Channel.csv"))
df_inventory = pd.read_csv(DATA_PATH.joinpath("Inventory.csv"))
df_booking_curve = pd.read_csv(DATA_PATH.joinpath("sample_data.csv"))
...
def update_dtd_booking_trend(start, end, forecast_range, market, clickData):
    ...
    df_res_1.rename(columns={'Departure_Status': 'Departure Type', 'NDO Range': 'Days-to-Departure Range'},
                    inplace=True)

    fig = px.bar(df_res_1, x='Days-to-Departure Range', y='Departures(%)', color='Departure Type', barmode='stack',
                 text='Departures(%)', custom_data=['Departure Type'], template='ggplot2')
    return fig
...
def update_mkt_booking_trend(start, end, forecast_range, market):
    ...
    fig = px.scatter(df_1, y="Yield Critical Departures (%)", x="Load Critical Departures (%)", size="Departures",
                     color="Route", custom_data=["Route"], template='ggplot2')
    return fig
...
def product_performance(clickData):
    ...
    figure = px.area(df_product_final, x='NDO', y='Load Factor', color='Product', template='ggplot2',
...
def journey_performance(clickData):
    ...
    figure = px.area(df_journey_final, x='NDO', y='Load Factor', color='Journey', template='ggplot2',
                     facet_row='Period', category_orders={'Period': ['Historical', 'Current']})
    figure.update_layout(legend=dict(
        orientation="h",
        # entrywidth=90,
